...
if ($conn->connect_error) {
    ...
    $sqlquery = "SELECT distinct authorid ,authorname,imageurl from playlist1;";
    $result = $conn->query($sqlquery);
    //echo "size of results array =".$results->num_rows;
    //$row['authorid']=1001;
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<div class="pull-left">';
                 echo '<img class="img-circle hidden-xs" src="'.$row['imageurl'].'" id="'.$row['authorid'].'" alt="Image not found" onError="this.onerror=null;this.src=\'https://api.fnkr.net/testimg/70x70/00CED1/FFF/?text=70x70\';" width="70px" height="72px"/>';
            echo '</div>';
            ...
            //titlebar
            echo '<table class="table table-condensed display tablesorter playList" id="'.$row['authorid'].'" cellspacing="0" width="100%" id="leftdashboardtable'.$row['authorid'].'">';
            echo '<thead>';
            echo '<tr>';
            echo '<th>#</th>';
            echo '<th>Title</th>';
            ...
            //body section begins
            echo '<tbody id="playlistItems'.$row['authorid'].'">';
            $authid=$row['authorid'];
            ...
            //include("rightDashboardStats.php");
